...
class PropertiesPanel {
  ...
  updateNodeInfo(node) {
    ...
    if (this.nodeName) {
      const customName = window.pluginManager?.getNodeDisplayName?.(
        node.type,
        definition.name,
      );
    ...
    try {
      ...
      if (!window._nodeCostsCache) {
        const response = await fetch(
          `${window.AIKAFLOW.apiUrl}/credits/node-costs.php`,
        );
      ...
      if (cost && cost > 0) {
        ...
        badge.className =
          "flex items-center gap-1 text-xs px-2 py-1 bg-yellow-500/10 text-yellow-400 rounded-lg mt-2";
        badge.innerHTML = `
  ...
  renderFields(node) {
    ...
    if (!definition) {
      this.content.innerHTML =
        '<p class="text-dark-400">No properties available.</p>';
      return;
    }
    ...
    fieldGroups.forEach((group) => {
      ...
      group.fields.forEach((field) => {
        ...
        let fieldCopy = { ...field };
        if (
          field.disabledWhenConnected &&
          connectedInputs.includes(field.disabledWhenConnected)
        ) {
  ...
  renderTextField(field, value, fieldId) {
    ...
    if (isMediaUrlField) {
      ...
      const isAudio =
        /\.(mp3|wav|ogg|m4a|aac)(\?.*)?$/i.test(lowerUrl) ||
  ...
  renderSelectField(field, value, fieldId) {
    const options = (field.options || [])
      .map((opt) => {
        const selected =
          (value || field.default) === opt.value ? "selected" : "";
    ...
    if (options.length === 0) {
      ...
      options.forEach((opt) => {
        ...
        const iconHtml = opt.icon
          ? `<i data-lucide="${opt.icon}" class="w-4 h-4 mr-2 opacity-60"></i>`
          : "";
        checkboxesHtml += `
  ...
  setupFieldListeners(nodeId) {
    ...
    this.content
      .querySelectorAll(
        'input[data-field-type="text"], textarea[data-field-type="textarea"]',
      )
      .forEach((input) => {
    ...
    // Number inputs
    this.content
      .querySelectorAll('input[data-field-type="number"]')
      .forEach((input) => {
    ...
    // Select inputs
    this.content
      .querySelectorAll('select[data-field-type="select"]')
      .forEach((select) => {
    ...
    // Checkbox inputs
    this.content
      .querySelectorAll('input[data-field-type="checkbox"]')
      .forEach((checkbox) => {
    ...
    // Slider inputs
    this.content
      .querySelectorAll('input[data-field-type="slider"]')
      .forEach((slider) => {
    ...
    // Color inputs
    this.content
      .querySelectorAll('input[data-field-type="color"]')
      .forEach((colorInput) => {
    ...
    // Color text inputs
    this.content
      .querySelectorAll('input[data-field-type="color-text"]')
      .forEach((textInput) => {
    ...
    this.content
      .querySelectorAll(".multiselect-container")
      .forEach((container) => {
        ...
        checkboxes.forEach((checkbox) => {
          checkbox.addEventListener("change", () => {
            ...
            container
              .querySelectorAll(
                `.multiselect-option[data-field-id="${fieldId}"]:checked`,
              )
              .forEach((cb) => {
    ...
    // Datetime inputs
    this.content
      .querySelectorAll('input[data-field-type="datetime"]')
      .forEach((input) => {
        input.addEventListener("change", (e) => {
          const value = e.target.value
    ...
    this.content.querySelectorAll(".file-dropzone").forEach((dropzone) => {
      ...
      const fileInput = this.content.querySelector(
        `input[data-field-id="${fieldId}"][type="file"]`,
      );
    ...
    this.content
      .querySelectorAll(".file-preview-enhance-btn")
      .forEach((btn) => {
  ...
  async handleFileUpload(nodeId, fieldId, file) {
    try {
      ...
      const fileData = {
        ...
        type: file.type,
        size: file.size,
        url: ensureHttps(result.url), // Server URL (CDN or local) with https://
  ...
  updateNodeField(nodeId, fieldId, value) {
    ...
    if (
      this.canvasManager &&
      (fieldId === "file" || fieldId === "url" || fieldId === "source")
    ) {
  ...
  showCustomPromptModal(text, textarea, actionBtn) {
    ...
    modal
      .querySelector(".custom-prompt-modal-close")
      .addEventListener("click", closeModal);
    modal
      .querySelector(".custom-prompt-cancel")
      .addEventListener("click", closeModal);
    ...
    // Submit handler
    modal
      .querySelector(".custom-prompt-submit")
      .addEventListener("click", async () => {
        const customPrompt = promptTextarea.value.trim();
        if (!customPrompt) {
          alert(
            window.t
              ? window.t("enhance.prompt_required")
        ...
        if (!text.trim()) {
          alert(
            window.t
              ? window.t("enhance.text_required")
              : "Please enter some text in the node first",
          );
          closeModal();
          return;
        }
        ...
        try {
          ...
          if (!deductData.success) {
            if (deductRes.status === 402) {
              throw new Error(
                `Insufficient credits. Required: ${deductData.required}, Balance: ${deductData.balance}`,
              );
            }
          ...
          const enhanced =
            await window.AIKAFLOWTextEnhance.enhanceWithCustomPrompt(
              text,
              customPrompt,
            );
          if (textarea) {
  ...
  showImageEnhanceModal(imageUrl, nodeId, fieldId) {
    ...
    submitBtn.addEventListener("click", async () => {
      ...
      if (!prompt) {
        if (window.Toast)
          Toast.error(
            window.t
              ? window.t("enhance.prompt_required")
              : "Please enter an enhancement prompt",
          );
        return;
      }
      ...
      submitBtn.innerHTML =
        '<i data-lucide="loader" class="w-4 h-4 animate-spin"></i> Enhancing...';
      ...
      try {
        ...
        const enhanced = await window.AIKAFLOWImageEnhance.enhance(
          imageUrl,
          prompt,
          "auto",
          updateStatus,
        ...
        modal
          .querySelector(".image-enhance-use")
          .addEventListener("click", () => {
            ...
            if (window.Toast)
              Toast.success(
                window.t
                  ? window.t("enhance.image_enhanced")
  ...
  setupLabelActions(definition, node) {
    ...
    definition.fields.forEach((field) => {
      ...
      const actionBtn = this.content.querySelector(
        `.label-action-btn[data-action="${field.labelAction.id}"]`,
      );
      if (!actionBtn) return;
      ...
      if (field.labelAction.id === "enhance") {
        actionBtn.addEventListener("click", async (e) => {
          ...
          // Handle item clicks
          dropdown
            .querySelectorAll(".enhance-dropdown-item")
            .forEach((item) => {
              item.addEventListener("click", async () => {
                const promptId = item.dataset.promptId || null;
                const textarea = this.content.querySelector(
                ...
                try {
                  ...
                  if (!deductData.success) {
                    ...
                    throw new Error(
                      deductData.error || "Failed to deduct credits",
                    );
                  }

                  const enhanced = await window.AIKAFLOWTextEnhance.enhance(
                    text,
                    promptId,
                  );
                  if (textarea) {
